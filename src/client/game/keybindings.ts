...
export const BINDING_GROUPS: {
  ...
  },
  {
    ...
    actions: [
      ...
      { action: 'shot', label: 'Tirar' },
      { action: 'shortPass', label: 'Pase corto' },
      { action: 'longPass', label: 'Pase largo / Deslizar' },
      { action: 'throughPass', label: 'Pase a través' },
      { action: 'switchPlayer', label: 'Cambiar jugador' },
      { action: 'contain', label: 'Contener (mantener)' },
    ],
  },
];
...
export function controlsLegend(b: KeyBindings): { keys: string; label: string }[] {
  return [
    ...
    { keys: codeLabel(b.shot), label: 'Tirar' },
    { keys: codeLabel(b.shortPass), label: 'Pase corto' },
    { keys: codeLabel(b.longPass), label: 'Pase largo / Deslizar' },
    { keys: codeLabel(b.throughPass), label: 'Pase a través' },
    {
      keys: `${codeLabel(b.switchPlayer)} + ${codeLabel(b.throughPass)}`,
      label: 'Lofted through ball',
    },
    { keys: codeLabel(b.contain), label: 'Contener (mantener)' },
    { keys: codeLabel(b.switchPlayer), label: 'Cambiar jugador' },
  ];
}
